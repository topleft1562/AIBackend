...
FunctionTool.from_defaults(
    ...
    description=(
        ...
        "**Required Fields:**\n"
        - `groupId`: int — the Telegram group ID\n"
        - `hostTelegramId`: int — the Telegram ID of the game creator\n"
        - `gameType`: str — free-form label (e.g. 'trivia', 'raffle', 'guess_the_price')\n"
        - `status`: str — 'in_progress', 'completed', or 'cancelled'\n"
        - `pointType`: str — 'ranking' or 'per_action'\n"
        - `pointValues`: dict —\n"
        "    → if 'ranking': { '1': 20, '2': 10, '3': 5 }\n"
        "    → if 'per_action': { 'correctAnswer': 5, 'participation': 1 }\n"
        - `players`: dict — keys are Telegram IDs, values can include { username, score, actions }\n"
        - `winners`: list — optional, fill once known (e.g. [{ telegramId, rank, points }])\n"
        - `createdAt`: ISODate\n\n"
