...
def handle_dispatch():
    ...
    "\n"
    "Your goal is to calculate EVERY possible valid route (any order, no repeats) that:\n"
    "- Starts at `start_location` and ends at `end_location`.\n"
    "- Chains one or more loads together.\n"
    ...
    "- Empty km = deadhead_km (start to A.pickup) "
    "+ deadhead_to_this_pickup (from reload option A for B) "
    "+ deadhead_to_this_pickup (from reload option B for C) "
    "+ return_km (from C.dropoff to end_location; this MUST always be included as empty km after the last load in the chain)\n"
    "- Loaded km = loaded_km for A + B + C\n"
    "\n"
    "**REMINDER:** You must always add the return_km (from the last load's dropoff to end_location) to empty km, even if the end_location is the same as the last dropoff (if so, return_km will be zero). Never omit this step."
    "\n"
    ...
    "After the city sequence, show: load_ids (in order), total loaded km, total empty km (including all deadheads and the return_km), loaded %, total km, total revenue, and RPM ($/mile)."
