...
def handle_dispatch():
    ...
    try:
        ...
        prompt = (
            ...
            "- Assign all loads using as few drivers as possible, but only where it creates a logical, efficient trip.\n"
            ...
            "- For each driver:\n"
            "    - Start at start_location, travel deadhead_km to first pickup.\n"
            "    - Chain multiple loads only when it is efficient. Use reload_option's deadhead_from_this_dropoff as empty km between chained loads.\n"
            "    - After the last dropoff, always travel return_km to end_location as empty km.\n"
            "    - Total empty km = first load's deadhead_km + reload_kms between chained loads + return_km after last dropoff.\n"
            "    - Only add the deadhead_km for the first load in a chain.\n"
            "    - When loads are chained, only add reload_km (deadhead_from_this_dropoff) between dropoff and next pickup.\n"
            ...
            "- Do NOT try to force every load into a trip. If a load doesn't fit efficiently, leave it unassigned.\n"
            ...
            "- Suggest any improvements or ways to group unassigned loads more efficiently if possible.\n\n"
