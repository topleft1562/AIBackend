...
def handle_dispatch():
    ...
    try:
        ...
        prompt = (
            ...
            "    • each route needs to include the deadhead kms for first trip, and return km's of last trip. as well as the deadhead_from_this_dropoff between each load as empty kms.\n"
            ...
            "- For each route, show the pickup and drop off cities in sequence including start and end points (brandon => winnipeg -> winnipeg => neepawa -> gladstone => brandon ** make loaded green and empty red), the loaded km, empty km, loaded %, total km, total revenue, and RPM ($/mile).\n"
            ...
            "\nAdditionally:\n"
            "- At the end, suggest strategic directions (city pairs or regions) where new loads would help create more 70%+ efficient routes, including possibilities for chaining in between loads. (for example: 'If you had loads between X and Y, you could connect A and B with high efficiency' ** be clear on the direction we need to find loads to fill the gaps) return this in clear HTML bullet points or chart style.\n"
            ...
            "    - load_id (int): Unique id\n"
            "    - pickup (string): Pickup city\n"
            "    - dropoff (string): Dropoff city\n"
            "    - revenue (float): Rate × weight for this load\n"
            "    - deadhead_km (float): Distance from start to pickup\n"
            "    - loaded_km (float): Pickup to dropoff distance\n"
            "    - return_km (float): Dropoff to end distance\n"
            "    - reload_options (dict): For chaining, shows possible next load pickups, deadhead_from_this_dropoff, loaded_km\n"
